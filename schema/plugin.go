...
type MachComposerPlugin interface {
	...
	SetGlobalConfig(meta Meta, data map[string]any) error

	// SetSiteConfig is called when the site config is set
	SetSiteConfig(site string, meta Meta, data map[string]any) error

	// SetSiteComponentConfig is called when the site component config is set
	SetSiteComponentConfig(site string, component string, meta Meta, data map[string]any) error
	...
	SetSiteEndpointConfig(site string, name string, meta Meta, data map[string]any) error

	// SetComponentConfig is called when the component config is set
	SetComponentConfig(component, version string, meta Meta, data map[string]any) error

	// SetComponentEndpointsConfig is called when the component endpoints config is set
	//
	// Deprecated: move endpoints to the terraform module instead
	SetComponentEndpointsConfig(component string, meta Meta, data map[string]string) error
